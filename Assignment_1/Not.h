...
class Not : public Binary { // the Binary is used as a subclass here
 public:
  // only accessors and mutators are public in this example
  Not();
  Not(int input1, int input2, std::bitset<32> bin_input1, std::bitset<32> bin_input2, std::bitset<32> Notoutput1, std::bitset<32> Notoutput2);
  ...
  std::bitset<32> get_Notoutput1();
  std::bitset<32> get_Notoutput2();

 private:
  // member variables are private and set/get via accessor/mutator
  int _input1;
  int _input2;
  std::bitset<32> _Notoutput1;
  std::bitset<32> _Notoutput2;
